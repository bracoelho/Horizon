...
**Why this is signal.** Coordinated disclosure has no window left. It gives maintainers days to weeks because an attacker was assumed to need comparable time to notice an issue and build against it. At ten minutes that assumption is gone.

**What it changes.** CVE assignment has stopped being a usable trigger. It runs three to four weeks behind, so a fixed release ships marked CVE-PENDING and your scanner reports nothing; track upstream advisories and patched releases directly. For a board, every control that assumes time between a fix existing and an exploit existing needs re-dating, and an OT estate on a quarterly patch cycle is where that assumption is most expensive.
